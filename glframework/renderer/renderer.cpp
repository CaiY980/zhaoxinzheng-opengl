...
Renderer::Renderer()
{
	...
	mCloudShader = new Shader("assets/shaders/cloud.vert", "assets/shaders/cloud.frag");
}
...
void Renderer::render(Scene* scene, Camera* camera, DirectionalLight* dirLight, AmbientLight* ambLight, unsigned int fbo) {
	...
	std::sort(
		...
		[camera](const Mesh* a, const Mesh* b) {
			...
			// 1. 计算网格 A 的世界坐标
			auto modelMatrixA = a->getModelMatrix(); // 获取网格 A 的模型矩阵（将模型从本地坐标系转换到世界坐标系）
			auto worldPositionA = modelMatrixA * glm::vec4(0.0, 0.0, 0.0, 1.0);
			// 说明：网格的本地原点（0,0,0）乘以模型矩阵，得到该原点在世界坐标系中的位置（世界坐标）

			// 2. 将世界坐标转换到相机坐标系
			auto cameraPositionA = viewMatrix * worldPositionA;
			// 结果：网格 A 的原点在相机坐标系中的位置，其中 .z 是相机系下的 Z 坐标
...
void Renderer::renderObject(Object* object, Camera* camera, DirectionalLight* dirLight, AmbientLight* ambLight) {
	...
	if (object-> getType() == ObjectType::Mesh || object->getType() == ObjectType::InstancedMesh)
	{
		...
		case MaterialType::GrassInstanceMaterial: {
			...
			break;
		case MaterialType::CloudMaterial: {
			CloudMaterial* cloudMat = (CloudMaterial*)material;
			...
			// 必要的环境参数
			shader->setVector3("cameraPosition", camera->mPosition);
			shader->setVector3("lightDirection", normalize(-dirLight->mDirection)); // 指向光源
			shader->setVector3("lightColor", dirLight->mColor);
			shader->setFloat("time", glfwGetTime());
			shader->setVector3("ambientColor", ambLight->mColor);
			// 材质参数
			shader->setFloat("cloudDensityThreshold", cloudMat->mDensityThreshold);
			shader->setFloat("cloudAbsorption", cloudMat->mAbsorption);
		}
										break;
